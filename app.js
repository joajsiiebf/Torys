...
function animateParticles() {
  ...
  particles.forEach(p => {
    ...
    ctx.fillStyle = 'rgba(24,119,242,0.7)';
    ctx.fill();
    p.y -= p.speed;
...
presenceRef.on('value', snap => {
  onlineCount.textContent = `Personas en línea: ${snap.numChildren()}`;
});
...
function renderTory(id, data) {
  ...
  div.className = 'toryCard';
  div.dataset.id = id;
  const date = new Date(data.timestamp || Date.now());
  const timeStr = date.toLocaleString();
  div.innerHTML = `<div class='timestamp'>${timeStr}</div><div>${data.text}</div>`;
  
  const nicknameBadge = document.createElement('div');
  nicknameBadge.className = 'nicknameBubbleCard';
  nicknameBadge.textContent = data.nickname || 'Anónimo';
  div.appendChild(nicknameBadge);
...
db.ref('torys').on('child_changed', snap => {
  ...
  if(div){
    ...
    ['👍','👎','😂','😡'].forEach((emoji,i)=>{
      actions.children[i].textContent = `${emoji} ${snap.val()[emoji]||0}`;
    });
  }
});
...
document.getElementById('notificationsBtn').addEventListener('click', () => alert('Notificaciones básicas activadas'));
document.getElementById('themeBtn').addEventListener('click', () => document.body.classList.toggle('darkTheme'));
document.getElementById('shareBtn').addEventListener('click', () => navigator.clipboard.writeText(location.href).then(()=>alert('Link copiado!')));
